...
class Where extends Command
{
    ...
    private function buildBrakets(\stdClass $brakets, Built $buildCommand, array $params)
    {
        // init sql for braket
        $sql = "";
        ...
        foreach ($brakets->childs as $child) {
            ...
            if ($child->type == 'brakets') {
                ...
                if ($child->value instanceof Command) {
                    $valueBuilt = $child->value->build($params);
                    $buildCommand->params($valueBuilt->params());

                    $s = "({$valueBuilt->command()})";
                } else {
                ...
                // Parse column
                $column = \Tiie\Data\functions::columnStr($child->column, $params);
    ...
    private function convForBetween($value, $command)
    {
        if (is_string($value)) {
            ...
            // $value = $t;
        // }elseif($value instanceof \Tiie\Data\Statements\Statement){
        //     $value = "({$value->slq()})";
        //     $command->merge($value->binds());
        // }elseif($value instanceof \Tiie\Data\Adapters\Commands\SQL\Select){
        //     $t = $value->build();
        //     $s = "({$t->slq()})";
